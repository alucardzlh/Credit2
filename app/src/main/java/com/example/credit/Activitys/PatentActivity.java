...
public class PatentActivity extends BaseActivity implements  PullToRefreshView.OnHeaderRefreshListener,PullToRefreshView.OnFooterRefreshListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        size = Integer.parseInt(DataManager.PatentInfoS.data.Paging.CurrentPage);
        ...
        handler=new Handler(){
            ...
            public void handleMessage(Message msg) {
                ...
                if(falg==true){
                    por=listPt.size()-1;
                    listPt.addAll(DataManager.PatentInfoSMore.data.patentInfo);
                    size++;
                }else{
                ...
                CC_List_itemAdapter adapter = new CC_List_itemAdapter(PatentActivity.this, list, "patent",listurl,null);
    ...
    public void onFooterRefresh(PullToRefreshView view) {
        mPullToRefreshView.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (Integer.parseInt(DataManager.PatentInfoS.data.Paging.TotalPage)>size){
                    falg = true;
                    GsonUtil request10 = new GsonUtil(URLconstant.URLINSER + URLconstant.PATENTURL, RequestMethod.GET);
                    request10.add("token", MD5.MD5s(DataManager.QJiugongGList.data.baseInfo.get(0).PRIPID +  new Build().MODEL));
                    request10.add("deviceId", new Build().MODEL);
                    request10.add("KeyNo", DataManager.QJiugongGList.data.baseInfo.get(0).PRIPID);
