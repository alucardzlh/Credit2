...
public class CommmentAdapter extends BaseAdapter {
    ...
    private List<DataManager.MyCommentlistr.DataBean.CommentInfoBean> list;
    ViewHolder vh = null;
    int sum=0;
    public CommmentAdapter(Context context, List<DataManager.MyCommentlistr.DataBean.CommentInfoBean> list) {
...
//@mipmap/nogood  差评 灰色
        String str="";
        if(!list.get(position).HEADICON.equals("")){
            str=((list.get(position).HEADICON).substring(list.get(position).HEADICON.length() - 20, list.get(position).HEADICON.length()-5)).replaceAll("\\/", "_");
...
//        }
         ...
         * 绑定评论的回复列表
         */
        if(DataManager.commentSonInfo!=null){
            vh.liuyan_num.setText(list.get(position).commentSonInfo.size()+"");
            Commment_ItemlistAdapter adapter=new Commment_ItemlistAdapter(context,list.get(position).commentSonInfo);
