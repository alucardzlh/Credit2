...
public class TrademarkActivity extends BaseActivity implements PullToRefreshView.OnHeaderRefreshListener, PullToRefreshView.OnFooterRefreshListener{
    ...
    boolean falg=false;
    int por;
    public static List<DataManager.trademarkModel.DataBean.BrandInfoBean> listTr = new ArrayList<>();//商标
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        size=Integer.parseInt(DataManager.trademarkModelS.data.Paging.CurrentPage);
        ...
        handler=new Handler(){
            ...
            public void handleMessage(Message msg) {
                ...
                if(falg==true){
                    por=listTr.size()-1;
                    listTr.addAll(DataManager.trademarkModelSMore.data.brandInfo);
                    size++;
                }else{
                    listTr=DataManager.trademarkModelS.data.brandInfo;
                }
                for(DataManager.trademarkModel.DataBean.BrandInfoBean p:listTr){
                    list.add(p.BRANDNAME);
                    if(!p.BRANDIMG.equals("") && p.BRANDIMG!=null){
                        listurl.add(p.BRANDIMG);
                    }
                }
                CC_List_itemAdapter adapter = new CC_List_itemAdapter(TrademarkActivity.this, list,"trademark",listurl,null);
    ...
    public void onFooterRefresh(PullToRefreshView view) {
        ...
        mPullToRefreshView.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if(Integer.parseInt(DataManager.trademarkModelS.data.Paging.TotalPage)>size){
                    falg=true;
                    GsonUtil request11 = new GsonUtil(URLconstant.URLINSER + URLconstant.TRADEMARKURL, RequestMethod.GET);
                    request11.add("token", MD5.MD5s(DataManager.QJiugongGList.data.baseInfo.get(0).PRIPID +  new Build().MODEL));
                    request11.add("deviceId", new Build().MODEL);
                    request11.add("KeyNo", DataManager.QJiugongGList.data.baseInfo.get(0).PRIPID);
