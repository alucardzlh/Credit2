...
//                startActivity(new Intent(SearchFirmActivty.this, MainActivity.class));
    ...
    public void Rit() {
        for (int i = 0; i < listpl.size(); i++) {
            if (!listpl.get(i).HEADICON.equals("")) {
                String strr = ((listpl.get(i).HEADICON).substring(listpl.get(i).HEADICON.length() - 20, listpl.get(i).HEADICON.length() - 5)).replaceAll("\\/", "_");
                File file = new File(Environment.getExternalStorageDirectory() + "/Credit/cache/" + strr + ".jpg");
                if (!file.exists()) {//获取本地图片路径是否存在
                    try {
                        BASE64Decoder decode = new BASE64Decoder();
                        byte[] b = decode.decodeBuffer(listpl.get(i).HEADICON);
