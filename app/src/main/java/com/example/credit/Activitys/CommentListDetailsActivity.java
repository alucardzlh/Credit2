...
public class CommentListDetailsActivity extends BaseActivity {
    ...
    String deviceId,pid,KeyNo,token;
    int S,N,So,No;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        builder2.setMessage("回复成功，请等待后台审核!!");
...
//                        DataManager.MyCommentlistr.DataBean.CommentInfoBean.CommentSonInfoBean r2=new DataManager.MyCommentlistr.DataBean.CommentInfoBean.CommentSonInfoBean();
//                        r2.MEMBERNAME=csp.getUSERNAME();
//                        r2.CONTENT=huifu_con.getText().toString();
//                        r2.CREATETIME=date;
//                        huifu_con.setText("");
//                        CommentListActivity.listpl.get(position).commentSonInfo.add(r2);
//                        Dliuyan_num.setText(CommentListActivity.listpl.get(position).commentSonInfo.size()+"");
//                        Rinit();
                        break;
                    case 3:
                        wd.dismiss();
    ...
    public void init(){
        ...
        if(!CommentListActivity.listpl.get(position).HEADICON.equals("")){
            commD_img.setImageBitmap(decodeBitmap(Environment.getExternalStorageDirectory() + "/Credit/cache/"+((CommentListActivity.listpl.get(position).HEADICON).substring(CommentListActivity.listpl.get(position).HEADICON.length() - 20, CommentListActivity.listpl.get(position).HEADICON.length()-5)).replaceAll("\\/", "_")+".jpg",35,35));
        }else{
    ...
    public void Rinit(){
        if(CommentListActivity.listpl.get(position).commentSonInfo!=null){
            Commment_ItemlistAdapter adapter=new Commment_ItemlistAdapter(CommentListDetailsActivity.this,CommentListActivity.listpl.get(position).commentSonInfo);
            ...
            switch (v.getId()){
                ...
                case R.id.Dhuifu_btn://发送键
                    if(!huifu_con.getText().toString().equals("")){
                        String KeyNo= DataManager.QJiugongGList.data.allCount.get(0).Pripid;
