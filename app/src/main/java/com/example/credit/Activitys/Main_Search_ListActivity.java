...
public class Main_Search_ListActivity extends BaseActivity implements PullToRefreshView.OnHeaderRefreshListener, PullToRefreshView.OnFooterRefreshListener{
    ...
    public static List<DataManager.trademarkModel.DataBean.BrandInfoBean> listsb = new ArrayList<>();//商标

    public static List<DataManager.zl_search.DataBean.PatentInfoBean> listzl = new ArrayList<>();//专利

    public static List<DataManager.MyDishonesty.DataBean.CourtcaseInfoBean> listsx = new ArrayList<>();//失信
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TotalPage=Integer.parseInt(i.getStringExtra("TotalPage"));
        CurrentPage=Integer.parseInt(i.getStringExtra("CurrentPage"));
        ...
        handler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    case 0://商标
                        if(!syatc.equals("1")){
                            for(DataManager.trademarkModel.DataBean.BrandInfoBean p:listsb){
                            ...
                            por=listsb.size()-1;
                            listsb.addAll(DataManager.trademarkModelSMore.data.brandInfo);
                            for(DataManager.trademarkModel.DataBean.BrandInfoBean p:listsb){
                                list.add(p.BRANDNAME);
                                if(!p.BRANDIMG.equals("") && p.BRANDIMG!=null){
                                    listurl.add(p.BRANDIMG);
                    ...
                    case 1://专利
                        if(!syatc.equals("1")) {
                            ...
                            por=listzl.size()-1;
                            listzl.addAll(DataManager.zl_searchSMore.data.patentInfo);
                            for (DataManager.zl_search.DataBean.PatentInfoBean p : listzl) {
                                list.add(p.PATENTNAME);
                                listurl.add(p.ABSTRACTGRAPH);
                            }
                    ...
                    case 2://失信人
                        if(!syatc.equals("1")) {
                            ...
                            por=listzl.size()-1;
                            listsx.addAll(DataManager.MyDishonestySMore.data.courtcaseInfo);
                            for (DataManager.MyDishonesty.DataBean.CourtcaseInfoBean p : listsx) {
                                list.add(p.INAME);
                            }
                            CurrentPage++;
