...
public class MycomplaintsListActivity extends BaseActivity implements  PullToRefreshView.OnHeaderRefreshListener,PullToRefreshView.OnFooterRefreshListener {
    ...
    LinearLayout Null;//空
    public static List<DataManager.MyComplaint.DataBean.ComplaintInfoBean> listmycp = new ArrayList<>();
    int falgg = 0;
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case 4://提交投诉后重新请求获取企业投诉数据源
                        ...
                        ComplaintsRuerst.add("token", MD5.MD5s(DataManager.QJiugongGList.data.baseInfo.get(0).PRIPID + ( new Build()).MODEL));
...
//                startActivity(new Intent(SearchFirmActivty.this, MainActivity.class));
        ...
        if (DataManager.myComplaint.data != null && !DataManager.myComplaint.data.equals(null) && DataManager.myComplaint.data.complaintInfo.size() != 0) {
    ...
    public void onFooterRefresh(PullToRefreshView view) {
        mPullToRefreshView.postDelayed(new Runnable() {
            @Override
            public void run() {
                if(Integer.parseInt(DataManager.myComplaint.data.Paging.TotalPage)>Integer.parseInt(DataManager.myComplaint.data.Paging.CurrentPage)){
