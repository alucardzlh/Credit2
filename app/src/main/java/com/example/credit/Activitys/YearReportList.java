...
public class YearReportList extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(DataManager.ZZGS.DataBean.AnbaseInfoBean p:DataManager.ZZGSModels.data.anbaseInfo){
            list.add(p.ANCHEYEAR);
        }
        CC_List_itemAdapter adapter = new CC_List_itemAdapter(YearReportList.this, list, "企业年报", null,null);
        YR_LV.setAdapter(adapter);
        YR_LV.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                startActivity(new Intent(YearReportList.this,WebAutonomy.class).putExtra("KeyNo",DataManager.ZZGSModels.data.anbaseInfo.get(position).ANCHEID).putExtra("priptype",DataManager.QJiugongGList.data.baseInfo.get(0).ENTTYPE));
            }
        });
