...
public class AdminActivity extends FragmentActivity implements View.OnClickListener {
    ...
    private void initDatas() {
        ...
        fragment.setListData(DataManager.AdminList.data.lic_Certificate,null);
        Admin_Fragment fragment1=Admin_Fragment.newInstance();
        fragment1.setListData(null,DataManager.AdminList.data.ot_Permit);//其它信息数据
    ...
    private void initViews() {
        ...
        if(DataManager.AdminList.data.lic_Certificate!=null && DataManager.AdminList.data.lic_Certificate.size()>0){
            xz_size1.setText(DataManager.AdminList.data.lic_Certificate.size()+"");
        }else{
            xz_size1.setText("0");
            xz_size1.setTextColor(getResources().getColor(R.color.huise));
        }
        if(DataManager.AdminList.data.ot_Permit!=null && DataManager.AdminList.data.ot_Permit.size()>0){
            xz_size2.setText(DataManager.AdminList.data.ot_Permit.size()+"");
        }else{
            xz_size2.setText("0");
